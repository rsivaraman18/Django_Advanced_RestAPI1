...
class UpdateReviewView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def perform_update(self, serializer):
        ...
        data1 = instance.watchlist
        reviewer_name = self.request.user
        
        # Ensure the user is the one updating their own review
        if instance.reviewer_name != reviewer_name:
            raise ValidationError('You can only update your own reviews.')
        
        # Handle the case where number_rating is zero
        if data1.number_rating == 0:
            # If there are no ratings, set the average rating to the new rating
            data1.avg_rating = serializer.validated_data['rating']
        else:
            # Calculate new average rating
            avg_cal = ((data1.avg_rating * data1.number_rating) - instance.rating + serializer.validated_data['rating']) / data1.number_rating
            data1.avg_rating = round(avg_cal, 3)
